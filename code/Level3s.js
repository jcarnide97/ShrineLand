class Level3s extends Phaser.Scene {
    constructor() {
        super('Level3s');
    }
    ...
    create() {
        ...
        this.characterP = this.physics.add.sprite(100, 430, 'popo');
        this.characterP.setBounce(0.2);
        this.characterP.setCollideWorldBounds(true);
        ...
        //colisões
        this.physics.add.collider(this.characterP, this.platforms);
        this.physics.add.collider(this.enemies, this.platforms);
        this.physics.add.collider(this.characterP, this.enemies, this.hitEnemies, null, this);
        this.physics.add.collider(this.characterP, this.shrine, this.reachShrine, null, this);
        this.physics.add.overlap(this.characterP, this.coins, this.collectCoins, null, this);
    }

    update() {
        if (this.controls.left.isDown) {
            this.characterP.setVelocityX(-160);
            this.characterP.anims.play('left', true);
        } else if (this.controls.right.isDown) {
            this.characterP.setVelocityX(160);
            this.characterP.anims.play('right', true);
        } else {
            this.characterP.setVelocityX(0);
            this.characterP.anims.play('turn');
        }

        if (this.controls.up.isDown && this.characterP.body.touching.down) {
            this.characterP.setVelocityY(-330);
        }
    }


    collectCoins(characterP, coins) {
    ...
    hitEnemies(characterP, enemies) {
        this.physics.pause();
        this.characterP.setTint(0xff0000);
        this.characterP.anims.play('turn');
        this.gameOver = true;
        this.scene.start('Level1s');
    }

    reachShrine(characterP, shrine) {
        this.physics.pause();
        this.characterP.setTint(0x0000ff);
        this.characterP.anims.play('turn');
        this.gameOver = true;
        this.scene.start('Level4s');
    }
}
