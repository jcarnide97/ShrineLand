class Level5s extends Phaser.Scene {
    constructor() {
        super('Level5s');
    }
    ...
    create() {
        ...
        this.characterM = this.physics.add.sprite(110, 430, 'megaMan');
        ...
        this.physics.add.collider(this.characterM, this.platforms);
        this.physics.add.collider(this.characterM, this.others, this.hitEnemies, null, this);
        this.physics.add.collider(this.characterM, this.shrine,this.reachShrine, null, this);
        this.physics.add.overlap(this.characterM, this.coins, this.collectCoins, null, this);
        this.physics.add.collider(this.enemies, this.platforms);
        this.physics.add.collider(this.characterM, this.enemies, this.hitEnemies, null, this);
        this.physics.add.collider(this.enemies, this.others);

        this.cameras.main.setBounds(0, 0, 1600, 600);
        this.cameras.main.startFollow(this.characterM);
    }


    update() {
        if (this.controlsC.left.isDown) {
            this.characterM.setVelocityX(-160);
            this.characterM.anims.play('left', true);
        } else if (this.controlsC.right.isDown) {
            this.characterM.setVelocityX(160);
            this.characterM.anims.play('right', true);
        } else {
            this.characterM.setVelocityX(0);
            this.characterM.anims.play('turn');
        }

        if (this.controlsC.up.isDown && this.characterM.body.touching.down) {
            this.characterM.setVelocityY(-320);
        }
    ...
    hitEnemies(characterM, enemies) {
        this.physics.pause();
        this.characterM.setTint(0xff0000);
        this.characterM.anims.play('turnC');
        this.gameOver = true;
        this.scene.start('Level1s');
    }


    reachShrine(characterM, shrine) {
        this.physics.pause();
        this.characterM.setTint(0x0000ff);
        this.characterM.anims.play('turnC');
        this.gameOver = true;
